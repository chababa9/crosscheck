...
export async function runClaudeReview(
  ...
  try {
    const { result: { stdout }, retried } = await withTimeoutRetry(
      ...
      (t) => execa('claude', args, { cwd: repoDir, timeout: t, input: prompt, env: { ...process.env } }),
      {
        onRetry: (timeoutMs, delayMs) =>
          onLog?.(`  ⏱ claude timed out at ${timeoutMs / 1000}s — waiting ${delayMs / 1000}s and retrying once`),
      },
    )
    ...
    const summary = execa.timedOut
      ? `timed out after ${effectiveMs !== undefined ? effectiveMs / 1000 : '?'}s (retried once) — PR diff may be too large`
